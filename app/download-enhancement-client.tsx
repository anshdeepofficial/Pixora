...
const CACHE_KEY = "pixora-server-enhance-cache-v1";
const POLL_MS = 1500;
...
function cacheId(url, scale) {
  return `${scale}x|${url}`;
}

function getCachedUrl(url, scale) {
  return readCache()[cacheId(url, scale)]?.url || "";
}
...
async function createServerEnhancement(imageUrl, scale) {
  ...
  if (!response.ok || !data.taskId) throw new Error(data.error || "Could not start server enhancement.");
  return data.taskId;
}

async function pollServerEnhancement(taskId, onStatus) {
  for (let attempt = 0; attempt < 160; attempt++) {
    if (attempt > 0) await wait(POLL_MS);
    const response = await fetch(`/api/enhance?id=${encodeURIComponent(taskId)}`, { cache: "no-store" });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "Could not check server enhancement.");
    if (data.status === "succeeded" && data.output?.[0]) return data.output[0];
    if (data.status === "failed") throw new Error(data.error || "Image enhancement failed.");
    onStatus(String(data.status || "processing"));
  }
  throw new Error("Server enhancement took too long. Please try again.");
}

async function enhancedUrl(sourceUrl, scale, onStatus) {
  ...
  onStatus("starting");
  const taskId = await createServerEnhancement(sourceUrl, scale);
  const output = await pollServerEnhancement(taskId, onStatus);
  rememberEnhancedUrl(sourceUrl, scale, output);
  return output;
}
...
function statusLabel(status) {
  if (status === "cached") return "Using saved server-enhanced result";
  if (/start|queue|pending/i.test(status)) return "Server model is starting";
  if (/process/i.test(status)) return "Server AI is restoring image detail";
  return "Server AI is processing the image";
}
...
export default function DownloadEnhancementClient() {
  ...
  const enhanced = async (scale) => {
    ...
    try {
      ...
      for (let index = 0; index < action.urls.length; index++) {
        ...
        const basePercent = (index / action.urls.length) * 100;
        setProgress({ percent: basePercent, label: `Starting server enhancement · ${number}/${action.urls.length}`, indeterminate: true });

        const outputUrl = await enhancedUrl(action.urls[index], scale, (status) => {
          setProgress({ percent: basePercent, label: `${statusLabel(status)} · ${number}/${action.urls.length}`, indeterminate: status !== "cached" });
        });

        setProgress({ percent: basePercent, label: `Fetching enhanced result · ${number}/${action.urls.length}`, indeterminate: true });
        const blob = await fetchImageBlob(outputUrl);
      ...
      if (zip) {
        ...
        save(archive, `pixora-server-enhanced-${scale}x-${action.urls.length}-images.zip`);
      }
  ...
  return <div className="pxEnhanceOverlay" role="dialog" aria-modal="true" aria-label="Download quality">
    ...
    <div className="pxEnhanceCard">
      ...
      <p className="pxEnhanceInfo"><b>Server-side AI</b><span>Heavy enhancement runs on VModel, not on your phone</span></p>
      <div className="pxEnhanceChoices">
        <button type="button" className="pxEnhanceChoice" disabled={busy} onClick={original}><span className="pxEnhanceIcon">1×</span><span><strong>Original</strong><small>Exactly as generated · no enhancement task</small></span><span className="pxEnhanceBadge">INSTANT</span></button>
        <button type="button" className="pxEnhanceChoice recommended" disabled={busy} onClick={() => enhanced(2)}><span className="pxEnhanceIcon">2×</span><span><strong>Server Enhanced HD</strong><small>GFPGAN v1.4 restores detail and resolution on the server</small></span><span className="pxEnhanceBadge">RECOMMENDED</span></button>
        <button type="button" className="pxEnhanceChoice" disabled={busy} onClick={() => enhanced(4)}><span className="pxEnhanceIcon">4×</span><span><strong>Server Enhanced Ultra</strong><small>Higher upscale factor · processed remotely so your device stays light</small></span><span className="pxEnhanceBadge">ULTRA</span></button>
      </div>
      {(busy || progress.percent > 0) && <div className="pxEnhanceProgress" aria-live="polite"><div className="pxEnhanceProgressTop"><span>{progress.label || "Preparing server enhancement"}</span><strong>{Math.round(progress.percent)}%</strong></div><div className={`pxEnhanceTrack ${progress.indeterminate ? "indeterminate" : ""}`}><i style={{ width: `${Math.max(0, Math.min(100, progress.percent))}%` }} /></div></div>}
      ...
      <p className="pxEnhanceFoot"><b>Usage:</b> each image enhanced uses one additional VModel GFPGAN enhancement task. These enhancement tasks do not increase Pixora's normal V-Editor generation counter. Successful enhanced results are saved and reused on this device when possible.</p>
    </div>
  </div>;
}
