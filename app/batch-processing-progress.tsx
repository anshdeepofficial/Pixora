...
export default function BatchProcessingProgress() {
      ...
      if (url.origin === window.location.origin && url.pathname === "/api/generate-batch" && method === "POST") {
        ...
        update({
          ...
          detail: sourceCount > 1
            ? `${sourceCount} uploaded images → ${expectedRequests} smart batch${expectedRequests === 1 ? "" : "es"}. Pair composites are being built and uploaded now.`
        ...
        try {
          ...
          if (response.ok || Array.isArray(data?.tasks)) {
            update({
              ...
              detail: `${realRequests} real VModel generation request${realRequests === 1 ? "" : "s"} accepted. Waiting for the model's actual task status.`,
              percent: null,
              sourceCount,
          ...
          if (data?.status === "failed") {
            ...
            update({
              ...
              detail: finished >= total
                ...
                : `Paired AI output has been separated and saved. Returning final images to Pixora · ${delivered}/${total}`,
            ...
            if (finished >= total) {
              ...
              hideTimer.current = window.setTimeout(() => {
                update({ visible: false, phase: "idle", title: "", detail: "", percent: null });
              }, 4200);
            }
          } else {
            update({
  ...
  return <div className="pxBatchProgress" role="status" aria-live="polite">
    <style>{`
      ...
      .pxBatchProgressHead{display:flex;justify-content:space-between;gap:12px;align-items:flex-start}.pxBatchProgressHead small{display:block;margin-bottom:4px;color:#7a7d76;font-size:9px;font-weight:800;letter-spacing:.09em;text-transform:uppercase}.pxBatchProgressHead strong{display:block;font:800 14px var(--font-manrope);letter-spacing:-.02em}.pxBatchRequest{flex:0 0 auto;padding:5px 8px;border-radius:99px;background:#eaf2d1;color:#40571d;font-size:9px;font-weight:850;white-space:nowrap}
      ...
      @media(max-width:620px){.pxBatchProgress{left:12px;right:12px;bottom:12px;width:auto;border-radius:16px;padding:14px}.pxBatchStages{gap:4px}.pxBatchStage{font-size:7.5px;padding:6px 3px}.pxBatchRequest{font-size:8px}}
    `}</style>
